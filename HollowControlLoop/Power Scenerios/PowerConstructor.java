...
public class PowerConstructor {
    ...
    public static void main (String[] args) throws Exception{
        ...
        int numCombinations = monitorDict.length *
            analyserDict.length * 
            plannerDict.length * 
            executorDict.length;
            ...
            for (Analyser a : analyserDict) {
                for (Planner p : plannerDict){
                    for (Executor e : executorDict){
                        ...
                        // GUID compatible and compilation error
                        if (result.compatible() &&
                            result.makespan() == -1){
                            compatibleAndFailCounter++;
                        }

                        // Not GUID compatible and compiled
                        if (!(result.compatible()) &&
                            result.makespan() != -1){
        ...
        // Print each simulation result ot csv
        ...
        Log.printlnConcat((double) compatibleCounter/(numCombinations) * 100, 
            "% of combinations are semantically compatible.");
        Log.printlnConcat((double) failCounter/(numCombinations) * 100, 
            "% of combinations could not be deployed.");
        if (compatibleCounter > 0) {
            Log.printlnConcat((double) compatibleAndFailCounter / compatibleCounter * 100, 
                "% of semantically compatible combinations could not be deployed.");
        }
        if (numCombinations - compatibleCounter > 0) {
            Log.printlnConcat((double) notCompatibleAndSucceededCounter / (numCombinations-compatibleCounter) * 100, 
                "% of non-semantically compatible combinations can be deployed.");
        }
    ...
    static void initCsv() throws IOException {
        csvWriter = new PrintWriter(new FileWriter("simulation_results_power.csv"));
    ...
    // Write given result to CSV file
