 ...
 */
public class ConstructorMultipleSims {
    ...
    private static final int NUM_SCENARIOS = 20;
    ...
    static void initCsv() throws IOException {
        ...
        csvWriter.println("compatible,monitor,analyser,planner,executor,scenario_seed,makespan,average_cpu_demand_variance,actionable_cycles,opportunity_cycles,actions_proposed,actions_executed,conversion_rate,runtime_ms,status");
    }
	...
	private static List<Vm> createVM(int userId, int vms, int idShift, long seed) {
    ...
    static void logResult(SimulationResult result, long scenarioSeed, long runtimeNanos) {
        ...
        csvWriter.printf("%b,%s,%s,%s,%s,%d,%.2f,%.6f,%d,%d,%d,%d,%.6f,%.3f,%s%n",
                result.compatible(),
                result.monitorId(),
                result.analyserId(),
                result.plannerId(),
