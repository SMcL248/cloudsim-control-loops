...
public class planner_v26 implements Planner<LoadState[], int[]> {
    ...
    public int[] plan(LoadState[] diagnosis, ReadSpace readSpace) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            double ramHeadroomRatio = readSpace.getHostTotalRam(host) > 0
                    ? readSpace.getHostAvailableRam(host) / readSpace.getHostTotalRam(host) : 1.0;
            double bwHeadroomRatio = readSpace.getHostTotalBw(host) > 0
                    ? readSpace.getHostAvailableBw(host) / readSpace.getHostTotalBw(host) : 1.0;
            if (ramHeadroomRatio >= bwHeadroomRatio) continue;
        ...
        Log.printlnConcat(readSpace.getNow(), ": [planner_v26] Constraint-Comparison RAM Scaling for VM ", vmId, " to tier ", tierIndex, ".");
        return new int[]{vmId, tierIndex};
    }

    @Override
    public String inputSemantic() {
        return "vm-loadstate-constraint-comparison-ram";
    }
