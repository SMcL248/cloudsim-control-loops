...
public class planner_v38 implements Planner<LoadState[], int[]> {
    ...
    public int[] plan(LoadState[] diagnosis, ReadSpace readSpace) {
        ...
        if (sourceHost != null) {
            ...
            if (!vmsOnHost.isEmpty()) {
                ...
                for (HostEntity h : hosts) {
                    ...
                    double mipsRatio = readSpace.getHostTotalMips(h) > 0
                            ? readSpace.getHostAvailableMips(h) / readSpace.getHostTotalMips(h) : 0;
                    double ramRatio = readSpace.getHostTotalRam(h) > 0
                            ? readSpace.getHostAvailableRam(h) / readSpace.getHostTotalRam(h) : 0;
                    double bwRatio = readSpace.getHostTotalBw(h) > 0
                            ? readSpace.getHostAvailableBw(h) / readSpace.getHostTotalBw(h) : 0;
                    double score = 0.5 * mipsRatio + 0.3 * ramRatio + 0.2 * bwRatio;
                    if (score > bestScore) {
                        bestScore = score;
        ...
        Log.printlnConcat(readSpace.getNow(), ": [planner_v38] Composite Weighted-Headroom Migration moving VM ", vmId, " to host ", targetHostId, ".");
        return new int[]{vmId, targetHostId};
    }

    @Override
    public String inputSemantic() {
        return "host-loadstate-composite-weighted-headroom";
    }
