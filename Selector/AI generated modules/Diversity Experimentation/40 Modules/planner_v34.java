...
public class planner_v34 implements Planner<LoadState[], int[]> {
    ...
    public int[] plan(LoadState[] diagnosis, ReadSpace readSpace) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (readSpace.isHostFailed(ownerHost) || readSpace.isHostPermanentlyDead(ownerHost)) {
                target = cl;
                sourceVm = ownerVm;
                break;
            }
        ...
        if (target != null) {
            ...
            for (GuestEntity vm : readSpace.getVmList()) {
                ...
                HostEntity h = null;
                for (HostEntity candidate : readSpace.getAllHosts()) {
                    if (readSpace.getVmListForHost(candidate).contains(vm)) { h = candidate; break; }
                }
                if (h == null || readSpace.isHostFailed(h) || readSpace.isHostPermanentlyDead(h)) continue;
        ...
        Log.printlnConcat(readSpace.getNow(), ": [planner_v34] Loss-Risk-Averse Migration salvaging cloudlet ", cloudletId, " from VM ", fromVmId, " on a failed host to VM ", toVmId, ".");
        return new int[]{cloudletId, fromVmId, toVmId};
    }

    @Override
    public String inputSemantic() {
        return "cloudlet-loadstate-loss-risk-averse";
    }
