...
public class monitor_v18 implements Monitor<double[]> {
    ...
    public double[] observe(ReadSpace readSpace) {
        ...
        for (int i = 0; i < cloudlets.size(); i++) {
            ...
            result[i] = total > 0 ? 1.0 - ((double) remaining / total) : 0.0;
        }
        Log.printlnConcat(readSpace.getNow(), ": [monitor_v18] computed completion fraction for ", cloudlets.size(), " cloudlets");
        return result;
    }

    @Override
    public String outputSemantic() {
        return "cloudlet-completion-fraction-ratio";
    }
