...
public class planner_v4 implements Planner<LoadState[], int[]> {
    ...
    public int[] plan(LoadState[] diagnosis, ReadSpace readSpace) {
        ...
        for (int i = 0; i < limit; i++) {
            ...
            int tierIndex = tierIndexOf(readSpace.getMipsTiers(), nextTierValue);
            if (tierIndex < 0) {
                continue;
            }
            int vmId = readSpace.getId(vm);
            Log.printlnConcat(readSpace.getNow(), ": [planner_v4] scaling VM ", vmId, " MIPS up to tier ", tierIndex);
            return new int[] { vmId, tierIndex };
        }

        Log.printlnConcat(readSpace.getNow(), ": [planner_v4] no overloaded VM eligible for MIPS scale-up");
        return new int[0];
    }
    ...
    public String inputSemantic() {
        return "vm-loadstate-cpu-overload";
    }
