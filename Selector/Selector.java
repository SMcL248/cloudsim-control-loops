...
public class Selector extends DatacenterBroker implements ActionSpace {
    ...
    private int loggingCycleCount = 0; 
    ...
    private final Set<Integer> loggedMissingAllocation = new HashSet<>();
    ...
    @Override
    // Cancel any outstanding events and shutdown the Selector
    ...
    @Override
    // Process completed tasks, end simulaiton if theres no outstanding work or failures
...
// -----------------------ActionSpace------------------------------
    ...
    @Override
    // Pause processing on given Cloudlet
    ...
    @Override
    // Resume processing on given Cloudlet
    ...
    @Override
    // Request the migration of the given VM to the given host
    ...
    public boolean requestPeDeallocation(GuestEntity vm){
        ...
        List<Double> currentShare = host.getGuestScheduler().getAllocatedMipsForGuest(vm);
        if (currentShare == null) {
            if (loggedMissingAllocation.add(vm.getId())) {
                Log.enable();
...
// -----------------------ReadSpace-------------------------------
    ...
    private void mapeCycle() {
        ...
        // System.out.println("CHECKPOINT A - before natural-completion check, t=" + getNow()
            // + " pendingInjections=" + pendingInjections
            // + " pendingFailures=" + pendingFailures
            // + " pendingRepairs=" + pendingRepairs
            // + " cloudletListEmpty=" + getCloudletList().isEmpty()
            // + " submitted=" + getCloudletSubmittedList().size()
            // + " received=" + getCloudletReceivedList().size());
        ...
        if (pendingInjections == 0 
            ...
            && getCloudletList().isEmpty() 
            && getCloudletSubmittedList().size() - numSubmittedCloudletsAbandoned == getCloudletReceivedList().size()) {
            // System.out.println("CHECKPOINT B - taking natural-completion early return, t=" + getNow());
            completedNaturally = true;
            return;
        }

        //System.out.println("CHECKPOINT C - past natural-completion check, t=" + getNow());
        Log.enable();
        updateKnowledge();
        Log.disable();
        //System.out.println("CHECKPOINT D - past updateKnowledge, t=" + getNow());
        updateSelected(); 
        //System.out.println("CHECKPOINT E - past updateSelected, selected=" + selected + ", t=" + getNow());

        // Debugging
        // System.out.println(energyExpelledReadings);
        // System.out.println("-------------------------------");
        // System.out.println(workCompleteReadings);
        // System.out.println("-------------------------------");
        // System.out.println(cloudletsAbandonedReadings);
        
        Log.enable();
        Log.printlnConcat(getNow(), ": [Selector] | Submitted: ", getCloudletSubmittedList().size(), " | Recieved: ", getCloudletReceivedList().size());
        
        Set<Integer> submittedIds = new HashSet<>();
        for (Cloudlet cl : getCloudletSubmittedList()) submittedIds.add(cl.getCloudletId());
        Set<Integer> receivedIds = new HashSet<>();
        for (Cloudlet cl : getCloudletReceivedList()) receivedIds.add(cl.getCloudletId());
        submittedIds.removeAll(receivedIds);

        // if (!submittedIds.isEmpty()){
        //     Log.printlnConcat(getNow(), ": [Selector] STUCK cloudlet IDs: ", submittedIds);
        //     for (int id : submittedIds){
        //         GuestEntity vm = getVmForCloudlet(id);
        //         Log.printlnConcat(getNow(), ": [Selector] Cloudlet #", id, " -> VM #", vm.getId(), " [MIPS: ", vm.getTotalMips(), ", Remaining MI: ", getCloudletById(id).getRemainingCloudletLength(), ", ETC: ",  getCloudletById(id).getRemainingCloudletLength()/vm.getTotalMips(), "]");
        //     }
        // }
        ...
        if (selected != null){ 
            try {
                ...
                e.printStackTrace(); // goes to System.err regardless of Log's enabled/disabled state
                Log.enable();
                Log.printlnConcat(getNow(), ": [Selector] EXCEPTION during observeAndAct: ",
                    e.getClass().getSimpleName(), " - ", e.getMessage());
                Log.disable();
                throw e; // rethrow -- don't change behavior, just make it visible before it's lost
            }
        } else {
            // System.out.println("CHECKPOINT E3 - selected is null, observeAndAct skipped entirely, t=" + getNow());
        }

        // System.out.println("CHECKPOINT F - past observeAndAct block, about to schedule next tick, t=" + getNow());
        // Schedule next observation
        schedule(getId(), observationRate, CloudActionTags.VM_BROKER_EVENT);
        // System.out.println("CHECKPOINT G - schedule() call returned, next tick should be t=" + (getNow() + observationRate));
    }
...
// Update the variance in demand across hosts, objectively captures current load-balance
    ...
    // Update RAM, BW and MIPS util. Increment sums for later mean calculations, and compare maximums
    ...
    public void registerInjection(double delay, List<Cloudlet> batch) {
        ...
        Log.enable();
        Log.printlnConcat(getNow(), ": ", batch.size(), " Cloudlets to be injected at time = ", delay);
        Log.disable();
    ...
    public void registerFailure(double delay, int hostId) {
        ...
        Log.enable();
        Log.printlnConcat(getNow(), ": Host #", hostId, " set to fail at time = ", delay);
        Log.disable();
        pendingFailures++;
    }
    ...
    private void logVmAllocation(ReadSpace readSpace) {
        ...
        for (HostEntity host : readSpace.getAllHosts()) {
            ...
            Log.enable();
            Log.printlnConcat(getNow(), "; [Selector] Host #", host.getId(), 
                ", Total MIPS: ", host.getTotalMips(), 
                ", Total RAM: ", host.getRam(), 
                ", Total BW: ", host.getBw(),
                ", Total Storage: ", host.getStorage()
            );
            Log.disable();
            if (guests.isEmpty()) {
        ...
        Log.enable();
        Log.printlnConcat(getNow(), ": Resource Util: ", vmMips/hostMips, "%");
        Log.disable();
    ...
    private GuestEntity getVmForCloudlet(int cloudletId){
        for (GuestEntity vm : getVmList()){
            for (Cloudlet cl : getVmCloudletList(vm)){
                if (getId(cl) == cloudletId){
                    return vm;
                }
            }
        }
    ...
    private static double lastOrNaN(List<Double> readings) {
        return readings.isEmpty() ? Double.NaN : readings.get(readings.size() - 1);
    }

    private void logGroundTruthCycle() {
        loggingCycleCount++;

        Log.printlnConcat(getNow(), ": [Selector][GT] demandVar=", lastOrNaN(demandVarianceReadings),
            " ramUtil=", lastOrNaN(ramUtilReadings), " bwUtil=", lastOrNaN(bwUtilReadings),
            " mipsUtil=", lastOrNaN(mipsUtilReadings), " storageUtil=", lastOrNaN(storageUtilReadings),
            " vmMipsUtil=", lastOrNaN(vmMipsUtilReadings),
            " ramUtilVar=", lastOrNaN(ramUtilVarianceReadings), " bwUtilVar=", lastOrNaN(bwUtilVarianceReadings),
            " mipsUtilVar=", lastOrNaN(mipsUtilVarianceReadings), " storageUtilVar=", lastOrNaN(storageUtilVarianceReadings),
            " vmMipsUtilVar=", lastOrNaN(vmMipsUtilVarianceReadings),
            " hostFreeMips=", lastOrNaN(hostFreeMipsReadings), " hostFreeRam=", lastOrNaN(hostFreeRamReadings),
            " hostFreeBw=", lastOrNaN(hostFreeBwReadings), " vmFreeMips=", lastOrNaN(vmFreeMipsReadings));

        if (loggingCycleCount % 10 == 0) {
            Log.printlnConcat(getNow(), ": [Selector][GT-avg10] demandVar=", rollingAverage(demandVarianceReadings, 10),
                " ramUtil=", rollingAverage(ramUtilReadings, 10), " bwUtil=", rollingAverage(bwUtilReadings, 10),
                " mipsUtil=", rollingAverage(mipsUtilReadings, 10), " storageUtil=", rollingAverage(storageUtilReadings, 10),
                " vmMipsUtil=", rollingAverage(vmMipsUtilReadings, 10),
                " ramUtilVar=", rollingAverage(ramUtilVarianceReadings, 10), " bwUtilVar=", rollingAverage(bwUtilVarianceReadings, 10),
                " mipsUtilVar=", rollingAverage(mipsUtilVarianceReadings, 10), " storageUtilVar=", rollingAverage(storageUtilVarianceReadings, 10),
                " vmMipsUtilVar=", rollingAverage(vmMipsUtilVarianceReadings, 10),
                " hostFreeMips=", rollingAverage(hostFreeMipsReadings, 10), " hostFreeRam=", rollingAverage(hostFreeRamReadings, 10),
                " hostFreeBw=", rollingAverage(hostFreeBwReadings, 10), " vmFreeMips=", rollingAverage(vmFreeMipsReadings, 10));
        }
    }

    private void logCloudletConservation(List<GuestEntity> vms){
        int execCount = 0, pausedCount = 0;
        for (GuestEntity vm : vms){
            execCount += vm.getCloudletScheduler().getCloudletExecList().size();
            pausedCount += vm.getCloudletScheduler().getCloudletPausedList().size();
        }
        int receivedCount = getCloudletReceivedList().size();
        int submittedCount = getCloudletSubmittedList().size();
        Log.enable();
        Log.printlnConcat(getNow(), ": [Selector][CONSERVE] exec=", execCount, " paused=", pausedCount,
            " received=", receivedCount, " abandoned=", numCloudletsAbandoned,
            " submitted=", submittedCount,
            " accounted=", (execCount + pausedCount + receivedCount + numCloudletsAbandoned),
            " vs submitted=", submittedCount);
        Log.disable();
    }
